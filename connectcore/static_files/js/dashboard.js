...
function createDevice(deviceData) {
    ...
    switch (deviceData[ID_DEVICE_TYPE]) {
        ...
        case CCIMX6ULSBC.DEVICE_TYPE:
            device = new CCIMX6ULSBC(getDeviceID(), deviceData);
            break;
        case CCMP157.DEVICE_TYPE:
            device = new CCMP157(getDeviceID(), deviceData);
            break;
        case CCMP133.DEVICE_TYPE:
            device = new CCMP133(getDeviceID(), deviceData);
            break;
        ...
        case CCIMX91.DEVICE_TYPE:
            device = new CCIMX91(getDeviceID(), deviceData);
            break;
        case CCIMX6SBC.DEVICE_TYPE:
            device = new CCIMX6SBC(getDeviceID(), deviceData);
            break;
        case CCIMX6QPSBC.DEVICE_TYPE:
            device = new CCIMX6QPSBC(getDeviceID(), deviceData);
            break;
    }
    if (device != null) {
        // Draw the device.
...
function initializeComponents() {
    ...
    if (device.getPCBColor() == ID_COLOR_GREEN)
        consolePanelArea.classList.add(CLASS_PANEL_BLUE);
